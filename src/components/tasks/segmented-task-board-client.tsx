...
export function SegmentedTaskBoardClient({ initialBoard }: SegmentedTaskBoardClientProps) {
  ...
  function syncTaskDraftState(nextBoard: SegmentedTaskBoard) {
    ...
    const validMainTaskIds = new Set<string>();
    nextBoard.segments.forEach((segment) => {
      validMainTaskIds.add(segment.id);
      segment.tasks.forEach((task) => {
        validTaskKeys.add(taskItemKey(segment.id, task.id));
      });
    });

    setExpandedMainTasks((previous) => {
      const next: Record<string, boolean> = {};
      Object.entries(previous).forEach(([key, value]) => {
        if (validMainTaskIds.has(key)) {
          next[key] = value;
        }
      });
      return next;
  ...
  function toggleMainTask(segmentId: string) {
    setExpandedMainTasks((previous) => ({
      ...previous,
    ...
    <div className="space-y-6">
      ...
      {board.segments.length ? (
        board.segments.map((segment) => {
          ...
          const isMainTaskExpanded = expandedMainTasks[segment.id] === true;
              ...
              <CardHeader className="border-b border-border/60 bg-muted/25 pb-4">
                <div className="flex flex-wrap items-center justify-between gap-3">
                  <div className="space-y-1">
                    ...
                    <p className="text-xs text-muted-foreground">{segment.tasks.length} task items</p>
                  </div>
                  <div className="flex items-center gap-2">
                    <Button
                      size="sm"
                      variant="secondary"
                      onClick={() => toggleMainTask(segment.id)}
                    >
                      {isMainTaskExpanded ? (
                      ...
                      {isMainTaskExpanded ? "Collapse" : "Open"}
                    </Button>
                    <Button
                      size="sm"
                      variant="secondary"
